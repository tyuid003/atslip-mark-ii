...
let txHistoryState = { userId: null, tenantId: null, page: 1, fromDate: '', toDate: '' };

async function openTransactionHistory(userId, tenantId, userName) {
  // ปิด pending menu
  document.querySelectorAll('.pending-item-dropdown').forEach(m => m.style.display = 'none');

  txHistoryState = { userId, tenantId, page: 1, fromDate: '', toDate: '' };

  // ตั้งวันที่เริ่มต้นเป็นวันนี้
  const today = new Date().toISOString().split('T')[0];
  txHistoryState.fromDate = today;
  txHistoryState.toDate = today;

  const modal = document.createElement('div');
  modal.className = 'tx-history-modal';
  modal.id = 'txHistoryModal';
  modal.innerHTML = `
    <div class="tx-history-content">
      <div class="tx-history-header">
        <h3>รายการฝาก-ถอน: ${userName || userId}</h3>
        <button class="tx-history-close" onclick="closeTxHistory()">
          <i data-lucide="x" style="width:20px;height:20px;"></i>
        </button>
      </div>
      <div class="tx-history-filters">
        <input type="date" id="txFromDate" value="${today}" />
        <input type="date" id="txToDate" value="${today}" />
        <button onclick="searchTxHistory()">ค้นหา</button>
      </div>
      <div class="tx-history-list" id="txHistoryList">
        <div class="tx-history-loading">กำลังโหลด...</div>
      </div>
      <div class="tx-history-pagination" id="txHistoryPagination" style="display:none;">
        <button id="txPrevBtn" onclick="txHistoryPrev()" disabled>ก่อนหน้า</button>
        <span class="tx-history-page-info" id="txPageInfo"></span>
        <button id="txNextBtn" onclick="txHistoryNext()">ถัดไป</button>
      </div>
    </div>
  `;
  ...
  modal.addEventListener('click', (e) => {
    if (e.target === modal) closeTxHistory();
  });
...
async function loadTxHistory() {
  ...
  const paginationEl = document.getElementById('txHistoryPagination');
  if (!listEl) return;

  listEl.innerHTML = '<div class="tx-history-loading">กำลังโหลด...</div>';

  try {
    const { userId, tenantId, page, fromDate, toDate } = txHistoryState;
    const result = await api.getUserTransactions(userId, tenantId, {
      page,
      limit: 20,
    ...
    if (transactions.length === 0) {
      listEl.innerHTML = '<div class="tx-history-empty">ไม่พบรายการ</div>';
    ...
    if (total > 20) {
      ...
      document.getElementById('txPageInfo').textContent = `หน้า ${page}/${totalPages} (${total} รายการ)`;
      ...
      document.getElementById('txPageInfo').textContent = `${total} รายการ`;
      document.getElementById('txPrevBtn').disabled = true;
      document.getElementById('txNextBtn').disabled = true;
    }
  } catch (error) {
    listEl.innerHTML = `<div class="tx-history-empty">เกิดข้อผิดพลาด: ${error.message}</div>`;
    paginationEl.style.display = 'none';
  }
}

function searchTxHistory() {
  txHistoryState.fromDate = document.getElementById('txFromDate')?.value || '';
  txHistoryState.toDate = document.getElementById('txToDate')?.value || '';
  txHistoryState.page = 1;
  loadTxHistory();
}

function txHistoryPrev() {
  if (txHistoryState.page > 1) {
    txHistoryState.page--;
    loadTxHistory();
  }
}

function txHistoryNext() {
  txHistoryState.page++;
  loadTxHistory();
}
...
window.searchTxHistory = searchTxHistory;
window.txHistoryPrev = txHistoryPrev;
window.txHistoryNext = txHistoryNext;
...
function resetSlipUpload() {
  ...
  if (dropzone) {
    dropzone.innerHTML = `
      ...
      <p>คลิกเพื่อเลือกไฟล์ หรือ คลิกในหน้าเว็บแล้วกด Ctrl+V</p>
    `;
    lucide.createIcons();
    bindUploadEvents();
  }
