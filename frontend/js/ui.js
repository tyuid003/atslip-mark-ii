    ...
    list.innerHTML = displayItems
      .map((item) => {
        ...
        const creditActionHtml = canWithdraw
          ...
          : (canCredit
              ? `<button class="pending-credit-btn" data-transaction-id="${item.id}" data-default-label="เติมเครดิต" onclick="creditPendingItem('${item.id}')" title="เติมเครดิต">เติมเครดิต</button>`
              : '');
